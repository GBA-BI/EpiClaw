#!/usr/bin/env python3
"""EpiClaw Metagenomics Classifier -- real-only execution pipeline."""
...
def summarize_fastq(path: Path) -> dict:
    def opener(file_path: Path, *args, **kwargs):
...
VERSION = "0.1.0"
SKILL_NAME = "metagenomics-classifier"
TOOL_CANDIDATES = ['kraken2', 'metaphlan']
DEMO_FASTQ = "@read1\nACGTACGTACGT\n+\nFFFFFFFFFFFF\n@read2\nTGCATGCATGCA\n+\nFFFFFFFFFFFF\n"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description="EpiClaw Metagenomics Classifier -- real execution only.",
    )
    parser.add_argument("--input", type=str, default=None, help="Input file or directory")
    parser.add_argument("--output", type=str, required=True, help="Output directory")
    parser.add_argument("--demo", action="store_true", help="Run built-in demo metagenomic reads")
    if TOOL_CANDIDATES:
...
def _write_demo_input(output_dir: Path) -> Path:
    demo_path = output_dir / "demo_metagenomics.fastq"
    demo_path.write_text(DEMO_FASTQ, encoding="utf-8")
    return demo_path
...
def generate_report(output_path: Path, input_path: Path, summary: dict, tool_info: dict) -> None:
    header = generate_report_header(
        title="Metagenomics Classifier Report",
...
def main(argv: list[str] | None = None) -> None:
    ...
    write_result_json(
        ...
        version=VERSION,
        summary={
            "n_reads": summary.get("n_reads", summary.get("n_sequences", 0)),
